...
<div class="row flex-grow">
    ...
    <div class="col-md-3">
        <p class="text-end">
            <a href="{{Route('supermarket.supplier')}}" class="btn btn-primary">
                <i class="fa-solid fa-plus"></i>
                supplier csv
            </a>
        </p>
...
<div class="row  mt-2">
    <div class="col-md-8  supermarket-grid">
        <table class="table table-dark">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <form action="{{route('remove', $Supermarkets->id)}}" method="post">
                            @csrf
                            @method('delete')
                            <button
                                style="background-color: darkcyan; margin: 1rem 0rem 0rem 4rem;padding: 1rem 2rem 1rem 2rem; border-radius: 4rem; ">Delete</button>
                        </form>
                    </td>
                </tr>
                @endforeach
...
<div class="row flex-grow mt-2">
    <div class="col-md-3">
        <div class="supermarket-list">
            @foreach ($Supermarket as $Supermarkets)
            <div class="item-list">
                <p>[{{ $Supermarkets->name }}</p>-<p>{{ $Supermarkets->location }}</p>-<p>{{ $Manager->id }}]</p>
            </div>
            @endforeach
        </div>
